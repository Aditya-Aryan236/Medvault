...
docker exec hospital-CHU-router mongosh --port 27017 --eval 'sh.addShard("sh1rs/shard1:27018")'
docker exec hospital-CHU-router mongosh --port 27017 --eval 'sh.addShard("sh2rs/shard2:27018")'

echo "✅ Sharded Cluster initialization completed!"

echo "2. Set shard key for patients collection (pre-split chunks)..."
docker exec hospital-CHU-router mongosh --port 27017 --eval '
